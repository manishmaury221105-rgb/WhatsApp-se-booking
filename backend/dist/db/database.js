...
const dbPath = process.env.DATABASE_PATH || path_1.default.join(__dirname, '../../whatsapp_booking.db');
...
function seedDefaultData() {
    const existingUser = exports.db.prepare('SELECT id FROM users LIMIT 1').get();
    if (existingUser)
        return;
    const adminPasswordHash = bcryptjs_1.default.hashSync('admin123', 10);
    const staffPasswordHash = bcryptjs_1.default.hashSync('staff123', 10);
  ...
  `);
    ...
    const insertBiz = exports.db.prepare(`
    INSERT INTO businesses (id, name, logo_url, phone, whatsapp_number, email, address, timezone, currency, description)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    insertBiz.run(businessId, 'Luxe Salon & Spa Studio', 'https://images.unsplash.com/photo-1560066984-138dadb4c035?w=200&auto=format&fit=crop', '+919876543210', '+919876543210', 'support@luxesalon.com', 'Shop 14, High Street Mall, Bandra West, Mumbai, MH 400050', 'Asia/Kolkata', '₹', 'Premium Hair, Skin, and Grooming services for men and women with instant AI booking on WhatsApp.');
    // Business Hours (0=Sun to 6=Sat)
    const insertHours = exports.db.prepare(`
  ...
  `);
    ...
    // Services
    const insertService = exports.db.prepare(`
  ...
  `);
    ...
    // Staff
    const insertStaff = exports.db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    insertStaff.run(staffId1, businessId, staffUserId1, 'Rahul Sharma', '+919876543211', 'rahul@salon.com', 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150', '[1,2,3,4,5,6,0]', '10:00', '19:00', '13:00', '14:00', 1);
    insertStaff.run(staffId2, businessId, staffUserId2, 'Priya Patel', '+919876543212', 'priya@salon.com', 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150', '[1,2,3,4,5,6]', '11:00', '20:00', '14:00', '15:00', 1);
    insertStaff.run(staffId3, businessId, null, 'Amit Verma', '+919876543213', 'amit@salon.com', 'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?w=150', '[1,2,3,4,5,0]', '09:30', '18:30', '13:30', '14:30', 1);
    // Staff Services
    const insertStaffService = exports.db.prepare(`
  ...
  `);
    ...
    // Message Templates
    const insertTemplate = exports.db.prepare(`
    INSERT INTO message_templates (id, business_id, template_type, name, content, variables, is_active)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
    insertTemplate.run('tmpl_01', businessId, 'BOOKING_CONFIRMATION', 'Booking Confirmation', '✅ *Appointment Confirmed!*\n\n*Booking ID:* {{booking_id}}\n*Service:* {{service_name}}\n*Date:* {{date}}\n*Time:* {{time}}\n*Staff:* {{staff_name}}\n*Price:* {{price}}\n\n*Location:* {{business_name}}\n\nThank you for choosing us! Looking forward to seeing you. 😊', JSON.stringify(['booking_id', 'service_name', 'date', 'time', 'staff_name', 'price', 'business_name']), 1);
    insertTemplate.run('tmpl_02', businessId, 'REMINDER_24H', '24-Hour Reminder', '🔔 *Appointment Reminder*\n\nHi {{customer_name}}, aapka appointment kal {{date}} ko *{{time}}* par {{service_name}} ke liye scheduled hai with {{staff_name}}.\n\nKoi change ho toh yahan reply karein.', JSON.stringify(['customer_name', 'date', 'time', 'service_name', 'staff_name']), 1);
    insertTemplate.run('tmpl_03', businessId, 'REMINDER_2H', '2-Hour Reminder', '🔔 *Reminder*\n\nHi {{customer_name}}, aapka appointment *2 hours* mein ({{time}}) scheduled hai at {{business_name}}.\n\nHum aapka wait kar rahe hain!', JSON.stringify(['customer_name', 'time', 'business_name']), 1);
    insertTemplate.run('tmpl_04', businessId, 'RESCHEDULE', 'Reschedule Confirmation', '🗓️ *Appointment Rescheduled*\n\n*Booking ID:* {{booking_id}}\n*New Date:* {{date}}\n*New Time:* {{time}}\n*Service:* {{service_name}}\n*Staff:* {{staff_name}}\n\nSee you soon at {{business_name}}!', JSON.stringify(['booking_id', 'date', 'time', 'service_name', 'staff_name', 'business_name']), 1);
    insertTemplate.run('tmpl_05', businessId, 'CANCELLATION', 'Cancellation Notice', '❌ *Appointment Cancelled*\n\n*Booking ID:* {{booking_id}}\n*Service:* {{service_name}}\n*Date:* {{date}}\n\nAapka appointment successfully cancel kar diya gaya hai. Agli baar book karne ke liye bas "Hi" bhejein!', JSON.stringify(['booking_id', 'service_name', 'date']), 1);
    insertTemplate.run('tmpl_06', businessId, 'FEEDBACK', 'Post-Service Feedback', '⭐ *Thank you for visiting {{business_name}}!* 😊\n\n{{customer_name}}, aapka {{service_name}} experience kaisa raha?\n\nPlease apna feedback share karein: 1 (Poor) se 5 (Excellent) ⭐', JSON.stringify(['business_name', 'customer_name', 'service_name']), 1);
    // Automation Rules
    const insertAutomation = exports.db.prepare(`
  ...
  `);
    ...
    // AI Agent Config
    const insertAIConfig = exports.db.prepare(`
  ...
  `);
    ...
    insertAIConfig.run('ai_cfg_01', businessId, 'Aria', 'Hinglish', 'Friendly', JSON.stringify(faqs), 'Appointments can be cancelled up to 2 hours before scheduled time without any fee.', 'Rescheduling is instant and free up to 1 hour before scheduled time.', 'Talk in friendly, polite Hindi/Hinglish. Use emojis. Never hallucinate unavailable slots or non-existing services. Always verify available slots using get_available_slots tool before promising a time.');
    // WhatsApp Config
    const insertWAConfig = exports.db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
    insertWAConfig.run('wa_cfg_01', businessId, '123456789012345', '987654321098765', 'EAAB_TEST_TOKEN_DEMO', 'whatsapp_booking_webhook_verify_token_2026', 1);
    // Sample Customers
    const insertCustomer = exports.db.prepare(`
  ...
  `);
    ...
    const insertAppt = exports.db.prepare(`
  ...
  `);
    ...
    // Notifications
    const insertNotif = exports.db.prepare(`
