...
class GeminiAiAgent {
    ...
    static async processMessage(params) {
        ...
        const aiConfig = database_js_1.db.prepare('SELECT * FROM ai_agent_configs WHERE business_id = ?').get(business_id);
        const business = database_js_1.db.prepare('SELECT * FROM businesses WHERE id = ?').get(business_id);
        // Load conversation history and state context
        const convRow = database_js_1.db.prepare('SELECT * FROM ai_conversations WHERE business_id = ? AND phone = ?').get(business_id, customer_phone);
        let history = [];
        let context = {};
        if (convRow) {
            try {
     ...
     */
    static async runGeminiWithTools(args) {
     ...
     * - FAQs answering
     */
    static async runDeterministicEngine(args) {
        ...
        const services = database_js_1.db.prepare('SELECT * FROM services WHERE business_id = ? AND is_active = 1').all(business_id);
        // Check FAQs first
        if (aiConfig && aiConfig.faqs_json) {
            try {
                const faqs = JSON.parse(aiConfig.faqs_json);
        ...
        if (msg.includes('cancel') || msg.includes('radd') || msg.includes('nahi aana')) {
            const activeAppt = database_js_1.db.prepare(`
        ...
        LIMIT 1
      `).get(business_id, customer_phone);
            if (!activeAppt) {
            ...
            if (context.pending_cancellation_id === activeAppt.id && (msg.includes('yes') || msg.includes('haan') || msg.includes('ha') || msg.includes('cancel kar do'))) {
                slotEngine_js_1.SlotEngine.cancelAppointmentTransaction({
                    business_id,
      ...
      `).get(business_id, customer_phone);
        ...
        if ((msg === 'yes' || msg === 'haan' || msg === 'ha' || msg === 'confirm' || msg.startsWith('yes ') || msg === 'thik hai') && context.selected_service_id && context.selected_date && context.selected_time) {
            try {
                const appt = slotEngine_js_1.SlotEngine.createBookingTransaction({
                    business_id,
                    customer_name,
                    customer_phone,
        ...
        if (context.selected_service_id && context.selected_date) {
            ...
            const availableSlots = slotEngine_js_1.SlotEngine.calculateAvailableSlots({
                business_id,
                date: context.selected_date,
                service_id: context.selected_service_id
            });
            ...
            if (selectedSlotTime) {
                ...
                const sTimeFmt = (0, slotEngine_js_1.formatTime12h)(selectedSlotTime);
                return {
...
exports.GeminiAiAgent = GeminiAiAgent;
