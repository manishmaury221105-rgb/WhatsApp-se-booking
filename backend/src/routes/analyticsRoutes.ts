...
router.get('/', authenticateToken, (req: AuthenticatedRequest, res: Response) => {
  ...
  const popularServices = db.prepare(`
    ...
    LIMIT 5
  `).all(businessId, businessId) as any[];

  // Customer acquisition (New vs Returning)
  const newCustomersRow = db.prepare('SELECT COUNT(*) as count FROM customers WHERE business_id = ? AND total_bookings = 1').get(businessId) as any;
  const returningCustomersRow = db.prepare('SELECT COUNT(*) as count FROM customers WHERE business_id = ? AND total_bookings > 1').get(businessId) as any;

  // Peak booking hours (Hour 9 to 20)
  const peakHoursRaw = db.prepare(`
