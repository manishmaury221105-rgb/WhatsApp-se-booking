...
import { WhatsAppConfig, MessageTemplate } from '../types/index.js';
import { v4 as uuidv4 } from 'uuid';
import { GeminiAiAgent } from './geminiAiAgent.js';
...
export class WhatsAppService {
   ...
   */
  public static getConfig(business_id: string): WhatsAppConfig | undefined {
    return db.prepare('SELECT * FROM whatsapp_configs WHERE business_id = ?').get(business_id) as WhatsAppConfig | undefined;
  }

  /**
   * Replace template variables: {{variable_name}} -> value
   */
  public static renderTemplate(content: string, vars: Record<string, string | number>): string {
    let result = content;
   ...
   */
  public static async sendMessage(options: SendMessageOptions): Promise<{ success: boolean; wa_message_id: string; error?: string }> {
    const { business_id, toPhone, messageText, messageType = 'text' } = options;
    const config = this.getConfig(business_id);
    const msgId = `wamid_${uuidv4().replace(/-/g, '')}`;
    ...
    `).run(`cnt_${uuidv4().substring(0, 8)}`, business_id, toPhone, toPhone);
   ...
   */
  public static async handleInboundMessage(params: {
    business_id: string;
    fromPhone: string;
    messageText: string;
    senderName?: string;
    waMessageId?: string;
  }): Promise<{ reply: string; appointment?: any }> {
    const { business_id, fromPhone, messageText, senderName, waMessageId } = params;

    // 1. Record Inbound Message
    const msgId = waMessageId || `wamid_in_${uuidv4().substring(0, 12)}`;
    db.prepare(`
      INSERT INTO whatsapp_messages (
        id, business_id, phone, direction, message_text, message_type, wa_message_id, status, raw_payload
    ...
    db.prepare(`
      ...
      ON CONFLICT(business_id, phone) DO UPDATE SET
        name = COALESCE(?, name),
        last_message_at = datetime('now')
    `).run(`cnt_${uuidv4().substring(0, 8)}`, business_id, fromPhone, senderName || 'WhatsApp Customer', senderName || null);

    // 3. Invoke Gemini AI Booking Agent with Tool Calling
    const aiResponse = await GeminiAiAgent.processMessage({
      business_id,
   ...
   */
  public static async sendAutomatedTemplate(
    business_id: string,
    toPhone: string,
    templateType: string,
    variables: Record<string, string | number>
  ): Promise<boolean> {
    const tmpl = db.prepare('SELECT * FROM message_templates WHERE business_id = ? AND template_type = ? AND is_active = 1').get(business_id, templateType) as MessageTemplate | undefined;
    if (!tmpl) {
